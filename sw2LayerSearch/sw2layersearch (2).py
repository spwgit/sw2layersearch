...
class sw2LayerSearch:
    ...
    def initGui(self):
        ...
        self.existingCheckAction = QAction(
            ...
            "Check Constraints For Existing Polygon SW2", self.tool_bar) # self.iface.mainWindow()
        self.freehandCheckAction = QAction(
            QIcon(":/plugins/sw2layerSearch/checker_freehand_32.png"),
            "Check Constraints For Free-hand Polygon SW2", self.tool_bar) # self.iface.mainWindow()
        ...
        self.iface.addPluginToMenu("&SW2 Constraint Checker", self.existingCheckAction)
        self.iface.addPluginToMenu("&SW2 Constraint Checker", self.freehandCheckAction)
        self.iface.addPluginToMenu("&SW2 Open CSV file", self.openCSVfileAction)
    ...
    def checkExistingGeometry(self):
        ...
        if selCount == 0: # only a single feature is valid
            iface.messageBar().pushMessage("Select features first...", level=QgsMessageBar.INFO, duration=5)
        elif selCount > 1:
            iface.messageBar().pushMessage("Selected features will be merged...", level=QgsMessageBar.INFO, duration=5)
    ...
    def checkFreehandGeometry(self):
        print("cfhg_triggered")
        self.iface.messageBar().pushMessage("Constraint Checker",    "Please digitise your area of interest - Right-click to add last vertex.",      level=QgsMessageBar.INFO, duration=10)
        self.iface.mapCanvas().setMapTool(self.freeHandTool)
                

    def unload(self):
        # Remove the plugin menu item and icon
        self.iface.removePluginMenu("&SW2 Constraint Checker", self.existingCheckAction)
        self.iface.removePluginMenu("&SW2 Constraint Checker", self.freehandCheckAction)
        self.iface.removePluginMenu("&SW2 Open CSV file", self.openCSVfileAction)
